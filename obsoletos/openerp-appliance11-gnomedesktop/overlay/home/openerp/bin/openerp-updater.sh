...
if [ "$SERVERNOPROMPTFORCE" = "on" -o "$CLIENTNOPROMPTFORCE" = "on" -o "$ADDONSNOPROMPTFORCE" = "on" -o "$EXTRAADDONSNOPROMPTFORCE" = "on" -o "$WEBCLIENTNOPROMPTFORCE" = "on" -o "$MAGENTONOPROMPTFORCE" = "on" -o "$REPORTOPENOFFICENOPROMPTFORCE" = "on" -o "$OPENETLNOPROMPTFORCE" = "on" ];
then
NEWREVISIONSFOUND=1
#FORCEUPDATE=1
LISTBRANCHES="openerp-server branch $SERVERNOPROMPTFORCE openerp-client branch $CLIENTNOPROMPTFORCE openerp-web branch $WEBCLIENTNOPROMPTFORCE Addons branch $ADDONSNOPROMPTFORCE extra-addons branch $EXTRAADDONSNOPROMPTFORCE magentoerpconnect branch $MAGENTONOPROMPTFORCE report-openoffice branch $REPORTOPENOFFICENOPROMPTFORCE openetl branch $OPENETLNOPROMPTFORCE"
else
...
case $? in
  ...
  1)
    ...
    LISTBRANCHES='openerp-server branch on openerp-client branch on openerp-web branch on Addons branch on extra-addons branch on magentoerpconnect branch on report-openoffice branch on openetl branch on'
...
then
        ...
        echo $"Usage without prompting: $0 {server|client|addons|extra-addons|extra|web-client|web|magentoerpconnect|report-openoffice|openetl|all}"
