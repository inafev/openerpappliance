...
SPAINNOPROMPTFORCE=off
MAGENTONOPROMPTFORCE=off
REPORTOPENOFFICENOPROMPTFORCE=off
OPENETLNOPROMPTFORCE=off
FORCEUPDATE=0
...
ADDONSSTATUS=7
EXTRAADDONSSTATUS=7
OPENERPSPAINSTATUS=7
MAGENTOERPCONNECTSTATUS=7
REPORTOPENOFFICESTATUS=7
OPENETLSTATUS=7
...
$DIALOG --backtitle "$background" \
		...
		"OpenERP Spain" "$OPENERPSPAINSTATUS" \
		"Magentoerpconnect" "$MAGENTOERPCONNECTSTATUS" \
		"report-openoffice" "$REPORTOPENOFFICESTATUS" \
		"OpenETL" "$OPENETLSTATUS" \
                #"Process nine" "-$i"
...
case $i in
  ...
  ;;
  50)
  cd /home/openerp/production/openerp-spain
  bzr missing > /tmp/check4newrevisions.txt 2>&1
  tail -1 /tmp/check4newrevisions.txt > /tmp/newrevisions.txt
  NEWREVISIONS=`awk '/Branches are up to date/ {print $0}' /tmp/newrevisions.txt`
  if [ -z "$NEWREVISIONS" ];
  then
    newrevisionsfound=1
    listbranches=$listbranches'openerp-spain branch on '
  else
    listbranches=$listbranches'openerp-spain branch off '
  fi
  OPENERPSPAINSTATUS=3;
  ;;
  70)
  cd /home/openerp/production/magentoerpconnect
  bzr missing > /tmp/check4newrevisions.txt 2>&1
  tail -1 /tmp/check4newrevisions.txt > /tmp/newrevisions.txt
  NEWREVISIONS=`awk '/Branches are up to date/ {print $0}' /tmp/newrevisions.txt`
  if [ -z "$NEWREVISIONS" ];
  then
    newrevisionsfound=1
    listbranches=$listbranches'magentoerpconnect branch on '
  else
    listbranches=$listbranches'magentoerpconnect branch off '
  fi
  MAGENTOERPCONNECTSTATUS=3;
  ;;
  80)
  cd /home/openerp/production/report-openoffice
  bzr missing > /tmp/check4newrevisions.txt 2>&1
  tail -1 /tmp/check4newrevisions.txt > /tmp/newrevisions.txt
  NEWREVISIONS=`awk '/Branches are up to date/ {print $0}' /tmp/newrevisions.txt`
  if [ -z "$NEWREVISIONS" ];
  then
    newrevisionsfound=1
    listbranches=$listbranches'report-openoffice branch on '
  else
    listbranches=$listbranches'report-openoffice branch off '
  fi
  REPORTOPENOFFICESTATUS=3;
  ;;
  90)
  cd /home/openerp/production/openetl
  bzr missing > /tmp/check4newrevisions.txt 2>&1
  tail -1 /tmp/check4newrevisions.txt > /tmp/newrevisions.txt
  NEWREVISIONS=`awk '/Branches are up to date/ {print $0}' /tmp/newrevisions.txt`
  if [ -z "$NEWREVISIONS" ];
  then
    newrevisionsfound=1
    listbranches=$listbranches'openetl branch on '
  else
    listbranches=$listbranches'openetl branch off '
  fi
  OPENETLSTATUS=3;
  ;;
esac
...
if [ "$SERVERNOPROMPTFORCE" = "on" -o "$CLIENTNOPROMPTFORCE" = "on" -o "$ADDONSNOPROMPTFORCE" = "on" -o "$EXTRAADDONSNOPROMPTFORCE" = "on" -o "$WEBCLIENTNOPROMPTFORCE" = "on" -o "$SPAINNOPROMPTFORCE" = "on" -o "$MAGENTONOPROMPTFORCE" = "on" -o "$REPORTOPENOFFICENOPROMPTFORCE" = "on" -o "$OPENETLNOPROMPTFORCE" = "on" ];
then
NEWREVISIONSFOUND=1
#FORCEUPDATE=1
LISTBRANCHES="openerp-server branch $SERVERNOPROMPTFORCE openerp-client branch $CLIENTNOPROMPTFORCE openerp-web branch $WEBCLIENTNOPROMPTFORCE Addons branch $ADDONSNOPROMPTFORCE extra-addons branch $EXTRAADDONSNOPROMPTFORCE openerp-spain branch $SPAINNOPROMPTFORCE magentoerpconnect branch $MAGENTONOPROMPTFORCE report-openoffice branch $REPORTOPENOFFICENOPROMPTFORCE openetl branch $OPENETLNOPROMPTFORCE"
else
...
case $? in
  ...
  1)
    ...
    LISTBRANCHES='openerp-server branch on openerp-client branch on openerp-web branch on Addons branch on extra-addons branch on openerp-spain branch on magentoerpconnect branch on report-openoffice branch on openetl branch on'
...
if [ "$SPAINNOPROMPTFORCE" = "on" ];
then
BRANCHESTOREINSTALL="$BRANCHESTOREINSTALL openerp-spain "
fi
if [ "$MAGENTONOPROMPTFORCE" = "on" ];
then
BRANCHESTOREINSTALL="$BRANCHESTOREINSTALL magentoerpconnect "
fi
if [ "$REPORTOPENOFFICENOPROMPTFORCE" = "on" ];
then
BRANCHESTOREINSTALL="$BRANCHESTOREINSTALL report-openoffice "
fi
if [ "$OPENETLNOPROMPTFORCE" = "on" ];
then
BRANCHESTOREINSTALL="$BRANCHESTOREINSTALL openetl "
fi
fi
...
NEWMAGENTOERPCONNECTREVISIONS=`echo $BRANCHESTOREINSTALL | awk '/magentoerpconnect/'`
if [ -n "$NEWMAGENTOERPCONNECTREVISIONS" ];
then
   NEWMAGENTOERPCONNECTREVISIONS=1
else
   NEWMAGENTOERPCONNECTREVISIONS=0
fi
...
for i in 5 10 20 30 40 50 60 70 75 80 85 90 100
do
$DIALOG --backtitle "$background" \
...
case $i in
  ...
  cd /home/openerp/production/openerp-spain
  bzr pull --overwrite >>$MYDESKTOP/OpenERP-updates.txt 2>&1
  bzr update /home/openerp/production/openerp-spain >>$MYDESKTOP/OpenERP-updates.txt 2>&1
  fi
  OPENERPSPAINSTATUS=3;
  ;;
  80)
  if [ $NEWMAGENTOERPCONNECTREVISIONS -eq 1 ];
  then
  cd /home/openerp/production/magentoerpconnect
  bzr pull --overwrite >>$MYDESKTOP/OpenERP-updates.txt 2>&1
  bzr update /home/openerp/production/magentoerpconnect >>$MYDESKTOP/OpenERP-updates.txt 2>&1
  fi
  MAGENTOERPCONNECTSTATUS=3;
  ;;
  85)
  ...
  cd /home/openerp/production/report-openoffice
  bzr pull --overwrite >>$MYDESKTOP/OpenERP-updates.txt 2>&1
  bzr update /home/openerp/production/report-openoffice >>$MYDESKTOP/OpenERP-updates.txt 2>&1
  fi
  REPORTOPENOFFICESTATUS=3;
  ;;
  90)
  ...
  cd /home/openerp/production/openetl
  bzr pull --overwrite >>$MYDESKTOP/OpenERP-updates.txt 2>&1
  bzr update /home/openerp/production/openetl >>$MYDESKTOP/OpenERP-updates.txt 2>&1
  fi
  OPENETLSTATUS=3;
  ;;
esac
...
OPENERPSPAININSTALLSTATUS=7 
MAGENTOERPCONNECTINSTALLSTATUS=7
REPORTOPENOFFICEINSTALLSTATUS=7
OPENETLINSTALLSTATUS=7 
MODULESUPDATESTATUS=7
CLIENTINSTALLSTATUS=7
WEBINSTALLSTATUS=7
STARTSTATUS=7

for i in 5 10 20 30 40 50 60 70 80 85 90 99 100
do
$DIALOG --backtitle "$background" \
        ...
        --mixedgauge "OpenERP-updates.txt file is created with records of installed or updated OpenERP revisions.This is useful for controlling your updates.\n\nCommand-line options: openerp-updater.sh --help" \
                ...
                "Reinstalling OpenERP Spain"             	"$OPENERPSPAININSTALLSTATUS" \
                "Reinstalling magentoerpconnect"             	"$MAGENTOERPCONNECTINSTALLSTATUS" \
                "Reinstalling report-openoffice"             	"$REPORTOPENOFFICEINSTALLSTATUS" \
                "Reinstalling openetl"             		"$OPENETLINSTALLSTATUS" \
...
case $i in
  ...
  if [ $NEWOPENERPSPAINREVISIONS -eq 1 ];
  then
  sudo ln -sf /home/openerp/production/openerp-spain/l10n_es/* $ADDONSPATH > /dev/null 2>&1
  sudo ln -sf /home/openerp/production/openerp-spain/l10n_es_extras/* $ADDONSPATH > /dev/null 2>&1
  sudo ln -sf /home/openerp/production/openerp-spain/extra_addons/* $ADDONSPATH > /dev/null 2>&1
  sudo ln -sf /home/openerp/production/openerp-spain/l10n_ca_ES/* $ADDONSPATH > /dev/null 2>&1
  [ -e /home/openerp/production/openerp-spain/l10n_es_ES/ ] && sudo ln -sf /home/openerp/production/openerp-spain/l10n_es_ES/* $ADDONSPATH >/dev/null 2>&1
  [ -e /home/openerp/production/openerp-spain/l10n_gl_ES/ ] && sudo ln -sf /home/openerp/production/openerp-spain/l10n_gl_ES/* $ADDONSPATH >/dev/null 2>&1
  [ -e /home/openerp/production/openerp-spain/l10n_eu_ES/ ] && sudo ln -sf /home/openerp/production/openerp-spain/l10n_eu_ES/* $ADDONSPATH >/dev/null 2>&1
  fi
  OPENERPSPAININSTALLSTATUS=3;
  ;;
  50)
  if [ $NEWMAGENTOERPCONNECTREVISIONS -eq 1 ];
  then
  sudo ln -sf /home/openerp/production/magentoerpconnect $ADDONSPATH > /dev/null 2>&1
  fi
  MAGENTOERPCONNECTINSTALLSTATUS=3;
  ;;
  60)
  if [ $NEWREPORTOPENOFFICEREVISIONS -eq 1 ];
  then
  sudo ln -sf /home/openerp/production/report-openoffice/* $ADDONSPATH > /dev/null 2>&1
  fi
  REPORTOPENOFFICEINSTALLSTATUS=3;
  ;;
  70)
  if [ $OPENETLINSTALLSTATUS -eq 1 ];
  then
  sudo ln -sf $ADDONSPATH/etl/lib/etl/ $SITEPACKAGESPATH/ > /dev/null 2>&1
  sudo ln -sf /home/openerp/production/openetl/lib/openetl/ $SITEPACKAGESPATH/ > /dev/null 2>&1
  sudo ln -sf /home/openerp/production/openetl/lib/etl_test/ $SITEPACKAGESPATH/ > /dev/null 2>&1
  ...
  #easy_install --install-dir $SITEPACKAGESPATH -U openerp-web > /dev/null 2>&1
  fi
  WEBINSTALLSTATUS=3;
  ;;
  99)
...
then
        ...
        echo $"Usage without prompting: $0 {server|client|addons|extra-addons|extra|web-client|web|openerp-spain|magentoerpconnect|report-openoffice|openetl|all}"
...
case "$arg" in
  ...
  web-client|web)
	WEBCLIENTNOPROMPTFORCE=on
        ;;
  openerp-spain)
	SPAINNOPROMPTFORCE=on
        ;;
  magentoerpconnect)
	MAGENTONOPROMPTFORCE=on
        ;;
  report-openoffice)
	REPORTOPENOFFICENOPROMPTFORCE=on
        ;;
  openetl)
	OPENETLNOPROMPTFORCE=on
        ;;
  all|All)
        SERVERNOPROMPTFORCE=on
	CLIENTNOPROMPTFORCE=on
