...
# This script eases the update of openerp v6.1 on OpenERP turnkeylinux based virtual appliance (http://www.turnkeylinux.org/)
# Script based on http://openerpappliance.com.wordpress.com/2009/09/15/openerp-all-in-one-installer-update-for-dummies/
...
declare -A BZR_BRANCH_WITH_MILESTONE_INFO[openerp-server]="openobject-server/6.1"
declare -A BZR_BRANCH_WITH_MILESTONE_INFO[openerp-client]="openobject-client/6.1"
declare -A BZR_BRANCH_WITH_MILESTONE_INFO[openerp-web]="openerp-web/6.1"
declare -A BZR_BRANCH_WITH_MILESTONE_INFO[addons]="openobject-addons/6.1"
...
{
  for b in openerp-server openerp-client openerp-web addons $COMMUNITYBRANCHES;
  do
...
{
  for b in openerp-server openerp-client openerp-web addons community-branches;
  do
...
{
COMMUNITYBRANCHES=""
STANDARDBRANCHES="openerp-server openerp-client openerp-web addons"
for dir in /home/openerp/*;
do
  if [ -d $dir ]; then
  dir=$(basename $dir)
...
function getLocalAndLaunchpadRevisionsPerBranchFunc()
{
  ...
  for br in openerp-server openerp-client openerp-web addons $COMMUNITYBRANCHES;
  do 
    cd /home/openerp/$br;
...
case $i in
  ...
  ######sudo ln -sf /home/openerp/addons/* $ADDONSPATH > /dev/null 2>&1
...
{
sudoFunc;
if [ "$SERVERNOPROMPTFORCE" = "on" -o "$CLIENTNOPROMPTFORCE" = "on" -o "$ADDONSNOPROMPTFORCE" = "on" -o "$WEBCLIENTNOPROMPTFORCE" = "on" -o "$COMMUNITYBRANCHESNOPROMPTFORCE" = "on" ];
then
NEWREVISIONSFOUND=1
#FORCEUPDATE=1
...
LISTBRANCHES="openerp-server ${LOCAL_MILESTONE_PER_BRANCH[openerp-server]} $SERVERNOPROMPTFORCE openerp-client ${LOCAL_MILESTONE_PER_BRANCH[openerp-client]} $CLIENTNOPROMPTFORCE openerp-web ${LOCAL_MILESTONE_PER_BRANCH[openerp-web]} $WEBCLIENTNOPROMPTFORCE Addons ${LOCAL_MILESTONE_PER_BRANCH[addons]} $ADDONSNOPROMPTFORCE community-branches ${LOCAL_MILESTONE_PER_BRANCH[community-branches]} $COMMUNITYBRANCHESNOPROMPTFORCE"
else
...
case $? in
  ...
  1)
    ...
    LISTBRANCHES="openerp-server ${LOCAL_MILESTONE_PER_BRANCH[openerp-server]} on openerp-client ${LOCAL_MILESTONE_PER_BRANCH[openerp-client]} on openerp-web ${LOCAL_MILESTONE_PER_BRANCH[openerp-web]} on Addons ${LOCAL_MILESTONE_PER_BRANCH[addons]} on community-branches ${LOCAL_MILESTONE_PER_BRANCH[community-branches]} on"
...
Example of how to add a new community branch before running this updater (openerp-spain, already added):\n
  1) cd /home/openerp/\n
  2) bzr branch lp:openerp-spain/6.1 openerp-spain\n
...
echo "  1) cd /home/openerp/" >>$MYDESKTOP/OpenERP-updates.txt
echo "  2) bzr branch lp:openerp-spain/6.1 openerp-spain" >>$MYDESKTOP/OpenERP-updates.txt
echo "  3) Update addons-path in /etc/openerp-server.conf by adding /home/openerp/openerp-spain" >>$MYDESKTOP/OpenERP-updates.txt
