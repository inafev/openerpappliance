...
declare -A BZR_BRANCH_WITH_MILESTONE_INFO[openerp-server]="openobject-server/6.0"
declare -A BZR_BRANCH_WITH_MILESTONE_INFO[openerp-client]="openobject-client/6.0"
declare -A BZR_BRANCH_WITH_MILESTONE_INFO[openerp-web]="openobject-client-web/6.0"
declare -A BZR_BRANCH_WITH_MILESTONE_INFO[addons]="openobject-addons/6.0"
...
then
        ...
        echo $"Usage without prompting: $0 {server|client|addons|extra-addons|extra|community-branches|web-client|web|all}"
...
case "$arg" in
  ...
  extra-addons|extra)
	EXTRAADDONSNOPROMPTFORCE=on
        ;;
  community-branches)
	COMMUNITYBRANCHESPROMPTFORCE=on
        ;;
  web-client|web)
